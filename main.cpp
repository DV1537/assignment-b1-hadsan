...
int main(int argc, const char * argv[])
{
  ...
  while(!file.eof())
    ...
    minimumRect = figure.getBoundingBox();

    std::cout << "The Bounding Box's minimum axis:\n";
	std::cout << "The top left corner: " << '(' << (minimumRect.tLCorner.xCoord * 1000) / 1000 << ',';
	std::cout << (minimumRect.tLCorner.yCoord * 1000) / 1000 << ')' << '\n';
	std::cout <<  "The bottom right corner: " << '(' << (minimumRect.bRCorner.xCoord * 1000) / 1000 << ',';
	std::cout << (minimumRect.bRCorner.yCoord * 1000) / 1000 << ')';
